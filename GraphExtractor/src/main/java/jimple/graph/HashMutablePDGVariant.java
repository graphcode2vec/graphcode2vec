...
public class HashMutablePDGVariant extends HashMutableEdgeLabelledDirectedGraph<PDGNode, String> implements ProgramDependenceGraph {
  ...
  public HashMutablePDGVariant(UnitGraph cfg) {
    ...
    // Construct the PDG
    this.constructPDG();
    this.m_pdgRegions = HashMutablePDGVariant.computePDGRegions(this.m_startNode);
        ...
        for (Iterator<Block> bItr = bs.iterator(); bItr.hasNext();) {
          ...
          if (b.equals(a)) {
           // throw new RuntimeException("PDG construction: A and B are not supposed to be the same node!");
        	 // System.out.println("Warning DG construction: A and B are not supposed to be the same node! ");
        	  continue;
          }
          ...
          while (dode != aParentDode) {
            ...
            if( dode == dode.getParent().getParent()) {
            	break;
            }
            dode = dode.getParent();
          ...
          if (regionOfB!=null && !this.m_obj2pdgNode.containsKey(regionOfB)) {
                  ...
                  while (succItr.hasNext()) {
                    ...
                    if(succRPDGNode.getType() == PDGNode.Type.REGION) {
                    	Region succR = (Region) succRPDGNode.getNode();
                    	 ...
                    	 //h =  (Block) succRPDGNode.getNode();
                    	// System.out.println("succRPDGNode.getNode is block");
                    	 continue;
                    }
                    assert h!=null;
